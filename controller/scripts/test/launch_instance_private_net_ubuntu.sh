...
source "$CONFIG_DIR/demo-openrc.sh"
...
function wait_for_service {
    ...
    until ssh_no_chk_node "$node" systemctl status "$service" | \
            grep -q "running"; do
        cnt=$((cnt + 1))
...
wait_for_service compute nova-compute
echo

function wait_for_nova_compute {

    echo "  Waiting for openstack-nova-compute service in state 'up'."
    if ssh_no_chk_node compute systemctl status nova-compute | \
            grep -q "running"; then
    ...
    until openstack compute service list --service nova-compute | grep -q "| up "; do
        ...
        if ssh_no_chk_node compute systemctl status nova-compute | \
            ...
            if [ $cnt -eq 300 ]; then
                ...
                echo "SUM ABORT openstack-nova-compute status remains down while service is up."
                echo "Aborting."
                exit 1
            fi
            echo -n .
...
function wait_for_nova_services {
    ...
    echo -n "  openstack-nova-compute"
    if ! openstack compute service list --service openstack-nova-compute | \
            grep -q '| up '; then
        wait_for_nova_compute
    fi
    echo
...
function show_compute_resource_usage {
    ...
    openstack host list
    echo "As admin user, openstack host show compute:"
    openstack host show compute
    )
}
...
function explain_instance_failure {
  ...
  The fastest way to update the database, however, is to restart openstack-nova-compute
  on the compute node.
